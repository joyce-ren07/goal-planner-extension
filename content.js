...
(function () {
  ...
  function findGCalLeftSidebarScrollEl() {
    ...
    for (const el of document.querySelectorAll('div')) {
      ...
      if (
        !/My calendars|Other calendars|Meet with|Booking pages|Booking insights?|Time insights?/i.test(tx)
      )
        continue;
      return el;
    }
  ...
  async function gpGdHydrateMountedDetailDecoration() {
    ...
    if (!gpGdHasRecentGoalChipOpenIntent()) {
      if (__gpGdBlockEl?.isConnected) teardownGpGdBlock();
    ...
    if (keep?.isConnected) {
      for (const h of natives) {
        ...
        if (gpGdIsValidEventDetailInspectorShell(h)) {
          gpGdStabilizeMountedDetailBlock(keep, h, null);
          gpGdMarkDetailScanActive(15000);
          return;
        }
      }
